	...
	public void onClick(View v) {
		if(v.getId()==R.id.xml_dcreate){
			try{
				...
				xml_res.setText("Dom����XML�ɹ�\n����:"+fName);
			}catch(Exception e){
				xml_res.setText("Dom����XMLʧ��");
				e.printStackTrace();
			}
			...
			try{
				...
				for(int i=0;i<nls.getLength();i++){
					...
					sb.append("--������:"+e.getElementsByTagName("name").item(0).getFirstChild().getNodeValue()+"\n");
					sb.append("--��ϵ�绰:"+e.getElementsByTagName("phone").item(0).getFirstChild().getNodeValue()+"\n");
					sb.append("--QQ:"+e.getElementsByTagName("qq").item(0).getFirstChild().getNodeValue()+"\n");
				}
				sb.deleteCharAt(sb.length()-1);
				xml_res.setText(sb.toString());
			}catch(Exception e){
				xml_res.setText("Dom����XMLʧ��!��ȷ��XML�ļ��Ѵ���");
				e.printStackTrace();
			}
